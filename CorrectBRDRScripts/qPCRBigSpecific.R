...
Beef = subset(metadata, cattle =="Beef") #471
...
BeefCID = subset(Beef, State =="ID") #225


BeefCCOPM= subset(BeefCCO, Serotype =="Pm-A")
sum <-BeefCCOPM %>%
...
BeefCIDPM= subset(BeefCID, Serotype =="Pm-A")
sum <-BeefCIDPM %>%
  group_by(Status) %>% # make one row per age
  # use group_by(species, age) if > 1 species
  summarize(
...
write.csv(sum, "BRDpathobiontAbundanceSerotypeBeef.csv")
#Prevalence graph for each bacterium.
# Indiana samples
library(dplyr)
summary <-BeefCCO%>%
...
summary2$State = "ID"
BeefPrev = rbind(summary, summary2)
...
Bact2PID = subset(Bact2IDT, pre_abs== "Positive")
Bact2PID$State = "ID"
BactSpe = rbind(Bact2PCO, Bact2PID)
