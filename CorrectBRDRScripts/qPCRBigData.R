...
metadata <- read.csv("./BigRF/BRDBigqPCRDairyTotal.csv", na.strings = c("","NA"), header=TRUE)
...
w = ggplot(metadata, aes(x=State, y=X16S.copies)) +
...
ggarrange(w,v,ncol =2, labels = c("A", "B"),
          font.label = list(size = 11))

##prepare the data for the loop, to identify the effect within state
IN= subset(metadata, State=="IN")
NY= subset(metadata, State=="NY") # change this to NY for dairy, ID for beef
CA= subset(metadata, State=="CA")# change this to CA for dairy, CO for beef
...
write.csv(ResultsTotal, "ResultsTotalDairy.csv")

#### Overall difference in 16S rRNA abundance within state
summary16 <-metadata%>%
...
summary16$State = as.factor(summary16$State)
summary16$comparison = paste(summary16$Status, summary16$State,sep = "-")
my_colors <- c("dodgerblue3","goldenrod3")

Sig16SState= summary16[c(5,6),]
write.csv(Sig16SState, "Sig16SStateDairy.csv")
str(metadata)
filtered_meta <- subset(metadata, State %in% c("NY"))

##Plot included the overall abundance of 16S abundance state ~ status
ggplot(filtered_meta, aes(x=State, y=X16S_log,color=Status)) +
  geom_point(aes(x=State,, y=X16S_log, color=Status),position = position_jitterdodge(0.4), alpha=0.2, size=1) +
  geom_errorbar(data=Sig16SState, aes(x=State, ymin=Mean - SE,
                              ymax=Mean + SE, y=NULL, color=Status), position = position_dodge(width=0.8), width=0.5) +
  scale_color_manual(values = my_colors) + theme_bw() +
  geom_point(data= Sig16SState, aes(State, y=Mean, color=Status), position=position_dodge(width = .8)) +
  ylab("16S rRNA Copy number (log10)")+
  theme(strip.text = element_text(size = 12, face = "bold")) +
  theme(legend.text = element_text(size=12)) +
  theme(legend.title = element_text(size = 12, face= "bold")) +
  theme(legend.key.size = unit(12, "point")) +
...
### for beef samples
metadata3 <- read.csv("BRDBigqPCRBeefTotal.csv", na.strings = c("","NA"), header=TRUE)
metadata3$Status = as.factor(metadata3$Status)
levels(metadata3$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
...
ggplot(metadata3, aes(x=Bacteria, y=copy_log,color=Status)) +
  geom_point(aes(x=Bacteria, y=copy_log, color=Status),position = position_jitterdodge(0.4), alpha=0.2, size=1) +
...
## Additional Dairy plot

#plot the results of the difference in BRD-pathobionts abundance within each state
summary <-metadata2%>%
  group_by(State, Bacteria, Status) %>% # make one row per age
  # use group_by(species, age) if > 1 species
  summarize(
...
##### ---------- Bacteria prevalence------------------
setwd("~/Desktop/eunice/PhD/AgSEED/BigProject/qPCRMetadata/")#sets new working directory for Windows systems (remember to replace … with your filepath)
