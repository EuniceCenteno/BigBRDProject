...
ASVs <- read_qza("~/Desktop/eunice/PhD/AgSEED/BigProject/BRDQiime/QiimeBeef/CorrectBeefQiime/table-no-pseudo.qza") #402 samples, but it will be 400 after removing Dairy 175, 176
ASV_s <- as.data.frame(ASVs$data)
ASV_table <- as.data.frame(ASVs$data)
ASV_table$ASVnos <- paste0("ASV", 1:nrow(ASV_table))
ASV_table$ASVstring <- rownames(ASV_table)
...
#change names on the ASV table 
ASV_table <- as.data.frame(ASV_table)
ASV_table <- merge(metadata, ASV_table, by.x = "ID", by.y = 0) #should be 400 samples
row.names(ASV_table) <- ASV_table$ID
ASV_table <- ASV_table[-c(1:21)]
ASV_table <- as.matrix(ASV_table)
...
##Adding taxonomy
#Taxonomy of each OTU
tax <- read_qza("~/Desktop/eunice/PhD/AgSEED/BigProject/BRDQiime/QiimeBeef/CorrectBeefQiime/taxonomyNoPseudo.qza")
tax <- as.data.frame(tax$data)
tax2 = separate(tax, Taxon, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus", "Species"), sep=";")
#This warning means that some cells are empty and that R is replacing empty cells with NA. Now there are other cells that are unclassified that  say, for example `s__` or `g__`. All these need to be removed and replaced with `NA`. 
#All this is OK except that in future use of the taxonomy table, these ASVs will be ignored because they are not classified. Why are ASVs not classified? Its because there is not a close enough match in the database. Just because there is not a good match in the database does not mean they don’t exist, so I wanted to make sure this data was not lost. So in my new code, from lines 300 – 334 I make it so that ASVs that are unclassified at any level are classified as the lowest taxonomic level for which there is a classification.
...
physeq_deseq %>%
  subset_taxa(Family != " Mitochondria" & 
                ...
                Genus != " Chloroplast" &
                Species != " Chloroplast") -> physeq_deseq
physeq_deseq #[ 25789 taxa and 487 samples ] Beef
...
patho$Species = as.factor(patho$Species)
levels(patho$Species)
patho$Genus = as.factor(patho$Genus)
levels(patho$Genus)
patho <- patho[patho$Species %in% c(" Mycoplasma"," Mycoplasma_sp.", " Pasteurella_multocida"," Pasteurella" ,
                                    " Histophilus"," Mannheimia", " Mannheimia_sp.", " Mannheimia_haemolytica"), ]
patho2 <- patho[patho$Genus %in% c(" Histophilus", " Mannheimia",  " Mycoplasma",  " Pasteurella"), ]

levels(patho$Species)
specific_myco = c(" Mycoplasma"," Mycoplasma_sp.")
myco = patho[patho$Species %in% specific_myco, ]

specific_paste = c(" Pasteurella_multocida"," Pasteurella")
Paste = patho[patho$Species %in% specific_paste, ]

specific_mann = c(" Mannheimia", " Mannheimia_sp.", " Mannheimia_haemolytica")
Mann = patho[patho$Species %in% specific_mann, ]

specific_histo = c(" Histophilus")
Histo = patho[patho$Species %in% specific_histo, ]

#at genus level
levels(patho2$Genus)
specific_myco = c(" Mycoplasma")
myco2 = patho2[patho2$Genus %in% specific_myco, ]

specific_paste = c(" Pasteurella")
Paste2 = patho2[patho2$Genus %in% specific_paste, ]

specific_mann = c(" Mannheimia")
Mann2 = patho2[patho2$Genus %in% specific_mann, ]

specific_histo = c(" Histophilus")
Histo2 = patho2[patho2$Genus %in% specific_histo, ]

NewASVTable2 = t(NewASVTable)
Myco_table <- as.data.frame(t(NewASVTable2[rownames(NewASVTable2) %in% rownames(myco2),]))
MycoSum = as.data.frame(rowSums(Myco_table))
Myco_table = merge(Myco_table, MycoSum, by.x = 0, by.y = 0)
str(Myco_table)
Myco_table$prevalence <- ifelse(Myco_table$`rowSums(Myco_table)` > 0, "Positive", "Negative")
...
Paste_table <- as.data.frame(t(NewASVTable2[rownames(NewASVTable2) %in% rownames(Paste2),]))
PasteSum = as.data.frame(rowSums(Paste_table))
Paste_table = merge(Paste_table, PasteSum, by.x = 0, by.y = 0)
str(Paste_table)
Paste_table$prevalence <- ifelse(Paste_table$`rowSums(Paste_table)` > 0, "Positive", "Negative")
...
PasteSum<- Paste_table %>% 
  group_by(State, Status, PMpre_abs) %>% 
  summarise (n = n()) %>%
  group_by(State, Status) %>%
  mutate(per =  100 *n/sum(n))
PasteSum$Bacteria = "P.multocida qPCR"
#PasteSumT = PasteSum
names(PasteSum)[names(PasteSum) == "PMpre_abs"] <- "prevalence"
PasteSumT = rbind(PasteSumT,PasteSum)
PasteSumTPos = subset(PasteSumT, prevalence== "Positive")

Mann_table <-as.data.frame(t(NewASVTable2[rownames(NewASVTable2) %in% rownames(Mann2),]))
MannSum = as.data.frame(rowSums(Mann_table))
Mann_table = merge(Mann_table, MannSum, by.x = 0, by.y = 0)
str(Mann_table)
Mann_table$prevalence <- ifelse(Mann_table$`rowSums(Mann_table)` > 0, "Positive", "Negative")
...
MannSum<- Mann_table %>% 
  group_by(State, Status, MHpre_abs) %>% 
  summarise (n = n()) %>%
  group_by(State, Status) %>%
  mutate(per =  100 *n/sum(n))
MannSum$Bacteria = "M. haemolytica qPCR"
#MannSumT = MannSum
names(MannSum)[names(MannSum) == "MHpre_abs"] <- "prevalence"
MannSumT = rbind(MannSumT,MannSum)
MannSumTPos = subset(MannSumT, prevalence== "Positive")

Histo_table <- as.data.frame(t(NewASVTable2[rownames(NewASVTable2) %in% rownames(Histo2),]))
HistoSum = as.data.frame(rowSums(Histo_table))
Histo_table = merge(Histo_table, HistoSum, by.x = 0, by.y = 0)
str(Histo_table)
Histo_table$prevalence <- ifelse(Histo_table$`rowSums(Histo_table)` > 0, "Positive", "Negative")
...
HistoSum<- Histo_table %>% 
  group_by(State, Status, HSpre_abs) %>% 
  summarise (n = n()) %>%
  group_by(State, Status) %>%
  mutate(per =  100 *n/sum(n))
HistoSum$Bacteria = "H. somni qPCR"
#HistoSumT = HistoSum
names(HistoSum)[names(HistoSum) == "HSpre_abs"] <- "prevalence"
HistoSumT = rbind(HistoSumT,HistoSum)
HistoSumTPos = subset(HistoSumT, prevalence== "Positive")

TotalPreBac = rbind(HistoSumTPos, PasteSumTPos, MannSumTPos, MycoSumTPos)
TotalPreBac2 = rbind(HistoSumTPos, PasteSumTPos, MannSumTPos, MycoSumTPos) #Genus
TotalPreBac2$Status = as.factor(TotalPreBac2$Status)
levels(TotalPreBac2$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
TotalPreBac2$Bacteria = as.factor(TotalPreBac2$Bacteria)

my_colors <- c("dodgerblue3","goldenrod3")
ggplot(TotalPreBac2, aes(x=Bacteria, y=per, fill=Status)) +
  geom_bar(stat="identity", position=position_dodge(), width=0.5)+
  scale_fill_manual(values = my_colors) +
  facet_grid(State~.)+ ylim(0,100) +
  theme_bw() + ylab("Prevalece Positive %") +
...
str(NewTax)
NewTax$Species = as.factor(NewTax$Species)
NewTax$Genus = as.factor(NewTax$Genus)
my_level <- c("Genus")
...
for(ml in my_level){
  ...
  taxa.summary <- physeq_bar_plot %>%
    ...
    psmelt()}  #%>%                               # Melt to long format
taxa <- taxa.summary %>% 
  group_by(Sample,  Status, Genus) %>% 
  summarise(taxa.sum = sum(Abundance)) %>%
  group_by(Sample, Status, Genus) %>%
  summarise(taxa.average = mean(taxa.sum)) ### relative abundance    
test = subset(taxa, Sample =="Beef101")
sum(test$taxa.average)
physeq.taxa.max <- taxa %>% 
  group_by(Sample, Genus) %>%
  summarise(overall.max=max(taxa.average))

test = subset(physeq.taxa.max, Sample =="Beef101")
sum(test$overall.max)
physeq.taxa.max <- as.data.frame(physeq.taxa.max)
# merging the phyla means with the metadata #
physeq_metaH <- merge(taxa, physeq.taxa.max)
physeq_metaH = as.data.frame(physeq_metaH)
write.csv(physeq_metaH, "physeq_meta_totalSpeciesHealthyBeefState.csv")
write.csv(physeq_metaH, "physeq_meta_totalGenusHealthyBeefState.csv")
HealthySpecies =  read.csv("physeq_meta_totalSpeciesHealthyBeefState.csv", na.strings = c("","NA"), header=TRUE)

#select the pathobionts
pathogen = as.factor(patho2$Genus)
#physeqmetaH = HealthySpecies #species
#physeqmetaH <- physeqmetaH[physeqmetaH$Species %in% c(" Mycoplasma"," Mycoplasma_sp.", " Pasteurella_multocida"," Pasteurella" ,
                                                      " Histophilus"," Mannheimia", " Mannheimia_sp.", " Mannheimia_haemolytica"), ]
physeqmetaH2 =physeq_metaH
physeqmetaH2 <- physeqmetaH2[physeqmetaH2$Genus %in% c(" Histophilus", " Mannheimia",  " Mycoplasma",  " Pasteurella"), ]
...
# merging the phyla means with the metadata #
physeq_metaB <- merge(taxa, physeq.taxa.max)
write.csv(physeq_metaB, "physeq_meta_totalSpeciesBRDBeefState.csv")
write.csv(physeq_metaB, "physeq_meta_totalGenusBRDBeefState.csv")

BRDSpecies =  read.csv("physeq_meta_totalSpeciesBRDBeefState.csv", na.strings = c("","NA"), header=TRUE)
physeqmetaB = BRDSpecies
physeqmetaB <- physeqmetaB[physeqmetaB$Species %in% c(" Mycoplasma"," Mycoplasma_sp.", " Pasteurella_multocida"," Pasteurella" ,
                                                      " Histophilus"," Mannheimia", " Mannheimia_sp.", " Mannheimia_haemolytica"), ]
physeqmetaB2 = physeq_metaB
physeqmetaB2 <- physeqmetaB2[physeqmetaB2$Genus %in% c(" Histophilus", " Mannheimia",  " Mycoplasma",  " Pasteurella"), ]

PathRel = rbind(physeqmetaB2, physeqmetaH2)
dplyr::count(PathRel, Sample) 
PathRel = PathRel[,-c(3)]
PathRel = merge(PathRel, metadata, by.x = "Sample", by.y = 0)
write.csv(PathRel, "PathRelGenus.csv")
...
###combine qPCR data and 16sabund
TotalMB = PathRel[PathRel$Genus %in% specific_myco, ]
TotalPM = PathRel[PathRel$Genus %in% specific_paste, ]
TotalHS = PathRel[PathRel$Genus %in% specific_histo, ]
TotalMH = PathRel[PathRel$Genus %in% specific_mann, ]
...
str(TotalMB)
cor(TotalMB$taxa.average, TotalMB$MBRel)
cor.test(TotalMB$taxa.average, TotalMB$MBRel) #not significant, significant using genus
...
d = ggscatter(TotalMB, x = "taxa.average", y = "MBRel",
  ...
  stat_cor(aes(color = Status), label.x = .05, label.y.npc = .4) 
d

#mannheimia
str(TotalMH)
cor(TotalMH$taxa.average, TotalMH$MHRel)
cor.test(TotalMH$taxa.average, TotalMH$MHRel) # no significant 
...
f = ggscatter(TotalMH, x = "taxa.average", y = "MHRel",
  ...
  stat_cor(aes(color = Status), label.x = .05, label.y.npc = .1) # Add correlation coefficient
f
