...
ASVs <- read_qza("table-no-pseudo.qza") #onlt 487 because they got lost in the sequencing part
ASV_s <- as.data.frame(ASVs$data)
ASV_table <- as.data.frame(ASVs$data)
ASV_table$ASVnos <- paste0("ASV", 1:nrow(ASV_table))
ASV_table$ASVstring <- rownames(ASV_table)
...
write.csv(Taxonomy, "BeefTaxonomy.csv")
...
BRD <- BRD[rownames(BRD) %in% rownames(B_OTU),] #220
healthy <- healthy[rownames(healthy) %in% rownames(H_OTU),] #239
...
for(ml in my_level){
  ...
  # merging the phyla means with the metadata #
  physeq_meta <- merge(taxa.summary, physeq.taxa.max)}
write.csv(physeq_meta, "physeq_meta_totalGenusHealthyBeef.csv")
physeq_meta_filtered <- filter(physeq_meta, overall.max>abund_filter)
...
# merging the phyla means with the metadata #
physeq_meta <- merge(taxa, physeq.taxa.max)
write.csv(physeq_meta, "physeq_meta_TotalGenusHealthyBeefState.csv")

physeq_meta_filtered <- filter(physeq_meta, overall.max>abund_filter)
write.csv(physeq_meta_filtered, "physeq_meta_filteredGenusHealthyBeefState.csv")
...
write.csv(physeq_meta, "physeq_meta_totalGenusBRDBeef.csv")
physeq_meta_filtered <- filter(physeq_meta, overall.max>abund_filter)
...
# merging the phyla means with the metadata #
physeq_meta <- merge(taxa, physeq.taxa.max)
write.csv(physeq_meta, "physeq_meta_totalGenusBRDBeefState.csv")
physeq_meta_filtered <- filter(physeq_meta, overall.max>abund_filter)
write.csv(physeq_meta_filtered, "physeq_meta_filteredGenusBRDBeefState.csv")

# we need the taxa information for the BRD and ehalthy animals
phylum <- read.csv("Taxonomy/PhylumBeef.csv", na.strings = c("","NA"), header=TRUE)
phylum$Status = as.factor(phylum$Status)
levels(phylum$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
...
family <- read.csv("Taxonomy/FamilyBeef.csv", na.strings = c("","NA"), header=TRUE)
family$Status = as.factor(family$Status)
levels(family$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
...
genus <- read.csv("Taxonomy/GenusBeef.csv", na.strings = c("","NA"), header=TRUE)
genus$Status = as.factor(genus$Status)
levels(genus$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
...
## Plot relative abundance per state
GenusBeef = read.csv("Taxonomy/GenusTotalStateBeef.csv", na.strings = c("","NA"), header=TRUE)

## now plot the difference 
my_colors <- c("dodgerblue3","goldenrod3")
str(GenusBeef)
GenusBeef$Genus = as.factor(GenusBeef$Genus)
GenusBeef$Status = as.factor(GenusBeef$Status)
levels(GenusBeef$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
levels(GenusBeef$Genus)

ggplot(data=GenusBeef, aes(x=State, y=taxa.average, fill=Status)) +
  geom_bar(stat="identity", position=position_dodge(), width=0.5)+
  scale_fill_manual(values = my_colors) + geom_hline(yintercept=0.02, linetype="dashed", color = "red", size=0.3) +
  facet_wrap(Genus~.)+ scale_y_continuous(breaks = seq(0, 0.28, by = 0.28), limits = c(0, 0.28)) +
  theme_bw() + ylab("Relative Abundance %") +
...
otu.summary <- prop.table((CO_OTU), 1) #accounts for the relative abundance in each sample
str(otu.summary)
otu_abund <- colSums(otu.summary) ##the abundance of each ASV across all samples
a <- as.data.frame(otu_abund)
sum(otu_abund)
...
otu.summary_sorted <- otu.summary[,order(otu.summary[1,], decreasing = TRUE)]
str(otu.summary_sorted)
melt_otu <- reshape2::melt(otu.summary_sorted[, c(1:25789)]) ###TOTAL NUMBER OF OTUS
str(melt_otu)
colnames(melt_otu) <- c("Sample", "ASV", "Abundance")
str(melt_otu)
levels(melt_otu$Sample)
...
Mh = subset(NewTax, Genus == " Mannheimia")
Mh$ASV = rownames(Mh)
repseqs <- read_qza("rep-seqs-nopseudoCor.qza") #onlt 487 because they got lost in the sequencing part
repseqs_s <- as.data.frame(repseqs$data)
Mh_seqs = merge(Mh, repseqs_s, by.x ="OTUs", by.y = 0)
#write.csv(Mh_seqs, "MhSequences.csv")
MhSimilar =  read.csv("MannheimaSimilarSeqs.csv", na.strings = c("","NA"), header=TRUE) #505
rownames(MhSimilar) = MhSimilar$OTUs
rownames(Mh) = Mh$OTUs
Mh <- Mh[rownames(Mh) %in% rownames(MhSimilar),] #select the sequences that are M. haemolytica
rownames(Mh) = Mh$ASV
Mh= Mh[-9]
...
patho$Species = as.factor(patho$Species)
levels(patho$Species)
patho <- patho[patho$Species %in% c(" Bibersteinia"," Bibersteinia_sp.", " Mesomycoplasma_dispar",
                                    " Mycoplasma"," Mycoplasma_sp.", " Mycoplasmopsis_bovirhinis", " Pasteurella_multocida",
                                    " Histophilus"," Mannheimia_sp."," Mannheimia", " Mannheimia_haemolytica", " Trueperella"), ]levels(patho$Species)

str(melt_otu)
meta_otu <- merge(COT, melt_otu, by.x = 0, by.y = "Sample") # change the metadata to whatever state you are working with
str(meta_otu)
...
### relative abundance of Mh similar sequences 
