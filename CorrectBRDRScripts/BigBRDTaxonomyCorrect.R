...
ASVs <- read_qza("table-no-pseudo.qza") #402 samples, but it will be 400 after removing Dairy 175, 176
...
my_level <- c("Genus")
my_column <- c("Status")  #this is the metadata column that we will use in the taxa barplot
...
write.csv(physeq_meta, "physeq_meta_totalGenusHealthyDairy.csv")
physeq_meta_filtered <- filter(physeq_meta, overall.max>abund_filter)
write.csv(physeq_meta_filtered, "physeq_meta_filteredGenusHealthyDairy.csv")
...
# merging the phyla means with the metadata #
physeq_meta <- merge(taxa, physeq.taxa.max)
write.csv(physeq_meta, "physeq_meta_TotalGenusHealthyDairyState.csv")

physeq_meta_filtered <- filter(physeq_meta, overall.max>abund_filter)
write.csv(physeq_meta_filtered, "physeq_meta_filteredGenusHealthyDairyState.csv")
...
write.csv(physeq_meta, "physeq_meta_totalGenusBRDDairy.csv")

physeq_meta_filtered <- filter(physeq_meta, overall.max>abund_filter)
write.csv(physeq_meta_filtered, "physeq_meta_filteredGenusBRDDairy.csv")

##Relative abundance by state
my_level <- c("Genus")
...
# merging the phyla means with the metadata #
physeq_meta <- merge(taxa, physeq.taxa.max)
write.csv(physeq_meta, "physeq_meta_totalGenusBRDDairyState.csv")
physeq_meta_filtered <- filter(physeq_meta, overall.max>abund_filter)
write.csv(physeq_meta_filtered, "physeq_meta_filteredGenusBRDDairyState.csv")

# we need the taxa information for the BRD and ehalthy animals
phylum <- read.csv("Taxonomy/PhylumDairy.csv", na.strings = c("","NA"), header=TRUE)
phylum$Status = as.factor(phylum$Status)
levels(phylum$Status) <- list("Healthy"="Healthy", "BRD"="BRD")

my_colors <- c("dodgerblue3","goldenrod3")

ggplot(phylum,aes(x=Abundance.average,y=factor(Phylum, levels = rev(levels(factor(Phylum)))),fill=Status)) + 
  geom_bar(stat = "identity", width= 0.7, position="dodge") + scale_fill_manual(values = my_colors) +
  theme_bw()+ #scale_x_continuous(breaks = seq(0, 0.30, by = 0.30), limits = c(0, 0.30)) +
  theme(strip.text.y = element_text(angle = 0, face="italic", size=12)) +theme(axis.title.y = element_blank()) +
  geom_vline(xintercept=0.02, linetype="dashed", color = "red", size=0.4) +ggtitle("Phylum") +
  theme( axis.title.x = element_text(color="black", size=11, face="bold")) + 
  theme(strip.text = element_text(size = 12, face = "bold")) +
  theme(legend.text = element_text(size=12)) +  xlab("Relative Abundance %") +
  theme(legend.title = element_text(size = 12, face= "bold")) +
  theme(legend.key.size = unit(12, "point")) +
  theme(axis.text.x = element_text(color = "black", size = 10), axis.text.y = element_text(color = "black", size = 12, face= "italic")) 

family <- read.csv("Taxonomy/FamilyDairy.csv", na.strings = c("","NA"), header=TRUE)
family$Status = as.factor(family$Status)
levels(family$Status) <- list("Healthy"="Healthy", "BRD"="BRD")

ggplot(family,aes(x=Abundance.average,y=factor(Family, levels = rev(levels(factor(Family)))),fill=Status)) + 
  geom_bar(stat = "identity", width= 0.7, position="dodge") + scale_fill_manual(values = my_colors) +
  theme_bw()+ #scale_x_continuous(breaks = seq(0, 0.30, by = 0.30), limits = c(0, 0.30)) +
  theme(strip.text.y = element_text(angle = 0, face="italic", size=10)) +theme(axis.title.y = element_blank()) +
  geom_vline(xintercept=0.02, linetype="dashed", color = "red", size=0.4) +ggtitle("Family") +
  theme( axis.title.x = element_text(color="black", size=11, face="bold")) + 
  theme(strip.text = element_text(size = 12, face = "bold")) +
  theme(legend.text = element_text(size=12)) + xlab("Relative Abundance %") +
  theme(legend.title = element_text(size = 12, face= "bold")) +
...
genus <- read.csv("Taxonomy/GenusDairy.csv", na.strings = c("","NA"), header=TRUE)
genus$Status = as.factor(genus$Status)
levels(genus$Status) <- list("Healthy"="Healthy", "BRD"="BRD")

ggplot(genus,aes(x=Abundance.average,y=factor(Genus, levels = rev(levels(factor(Genus)))),fill=Status)) + 
  geom_bar(stat = "identity", width= 0.7, position="dodge") + scale_fill_manual(values = my_colors) +
  theme_bw()+ #scale_x_continuous(breaks = seq(0, 0.30, by = 0.30), limits = c(0, 0.30)) +
  theme(strip.text.y = element_text(angle = 0, face="italic", size=10)) +theme(axis.title.y = element_blank()) +
  geom_vline(xintercept=0.02, linetype="dashed", color = "red", size=0.4)  +ggtitle("Genus") +
  theme( axis.title.x = element_text(color="black", size=11, face="bold")) + 
  theme(strip.text = element_text(size = 12, face = "bold")) +
  theme(legend.text = element_text(size=12)) + xlab("Relative Abundance %") +
  theme(legend.title = element_text(size = 12, face= "bold")) +
...
## Plot relative abundance per state
GenusDairy = read.csv("GenusTotalStateDairy.csv", na.strings = c("","NA"), header=TRUE)

## now plot the difference 
my_colors <- c("dodgerblue3","goldenrod3")
str(GenusDairy)
GenusDairy$Genus = as.factor(GenusDairy$Genus)
GenusDairy$Status = as.factor(GenusDairy$Status)
levels(GenusDairy$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
levels(GenusDairy$Genus)

ggplot(data=GenusDairy, aes(x=State, y=taxa.average, fill=Status)) +
  geom_bar(stat="identity", position=position_dodge(), width=0.5)+
  scale_fill_manual(values = my_colors) + geom_hline(yintercept=0.02, linetype="dashed", color = "red", size=0.3) +
  facet_wrap(Genus~.)+ scale_y_continuous(breaks = seq(0, 0.28, by = 0.28), limits = c(0, 0.28)) +
...
patho <- patho[patho$Species %in% c(" Bibersteinia_trehalosi", " Mesomycoplasma_dispar",
                                    " Mycoplasma", " Mycoplasmopsis_bovirhinis", " Pasteurella_multocida",
                                    " Histophilus", " Mannheimia_haemolytica", " Trueperella"), ]
levels(patho$Species)
...
write.csv(SpeciesS, "DairyStateBRDSpecies.csv")
...
###beef samples
metadata <- read.csv("BRDBigqPCRBeefTotalNamesRow.csv", na.strings = c("","NA"), header=TRUE) #505
row.names(metadata) <- metadata$ID
order_groups <- metadata$ID
metadata$Status <- as.factor(metadata$Status)
dplyr::count(metadata, Status) 
...
ASVs <- read_qza("table-no-pseudo.qza") #onlt 498 because they got lost in the sequencing part
ASV_s <- as.data.frame(ASVs$data)
ASV_table <- as.data.frame(ASVs$data)
ASV_table$ASVnos <- paste0("ASV", 1:nrow(ASV_table))
ASV_table$ASVstring <- rownames(ASV_table)
...
#We then merge these into an object of class phyloseq.
physeq_deseq = phyloseq(OTU.physeq, tax.physeq, meta.physeq)
physeq_deseq # [ 26496 taxa and 459 samples ] beef
...
physeq_deseq %>%
  subset_taxa(Family != " Mitochondria" & 
                ...
                Genus != " Chloroplast" &
                Species != " Chloroplast") -> physeq_deseq
physeq_deseq #[ 26365 taxa and 459 samples ]
...
BRD <- BRD[rownames(BRD) %in% rownames(B_OTU),] #239
healthy <- healthy[rownames(healthy) %in% rownames(H_OTU),] #220
...
for(ml in my_level){
  ...
  physeq_meta_filtered <- filter(physeq_meta, overall.max>abund_filter)}
write.csv(physeq_meta_filtered, "physeq_meta_filteredGenusHealthyBeef.csv")
...
write.csv(physeq_meta_filtered, "physeq_meta_filteredGenusBRDBeef.csv")

# we need the taxa information for the BRD and ehalthy animals
phylum <- read.csv("PhylumBeef.csv", na.strings = c("","NA"), header=TRUE)
phylum$Status = as.factor(phylum$Status)
levels(phylum$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
...
ggplot(phylum,aes(x=Abundance.average,y=factor(Phylum, levels = rev(levels(factor(Phylum)))),fill=Status)) + 
  ...
  theme_bw()+ #scale_x_continuous(breaks = seq(0, 0.30, by = 0.30), limits = c(0, 0.30)) +
  theme(strip.text.y = element_text(angle = 0, face="italic", size=12)) +theme(axis.title.y = element_blank()) +
  xlab(paste0("Relative Abundance > 2% per samples")) +ggtitle("Phylum") +
  theme( axis.title.x = element_text(color="black", size=11, face="bold")) + 
  theme(strip.text = element_text(size = 12, face = "bold")) +
  theme(legend.text = element_text(size=12)) +
  theme(legend.title = element_text(size = 12, face= "bold")) +
...
family <- read.csv("FamilyBeef.csv", na.strings = c("","NA"), header=TRUE)
family$Status = as.factor(family$Status)
levels(family$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
...
ggplot(family,aes(x=Abundance.average,y=factor(Family, levels = rev(levels(factor(Family)))),fill=Status)) + 
  ...
  theme_bw()+ #scale_x_continuous(breaks = seq(0, 0.30, by = 0.30), limits = c(0, 0.30)) +
  theme(strip.text.y = element_text(angle = 0, face="italic", size=10)) +theme(axis.title.y = element_blank()) +
  xlab(paste0("Relative Abundance > 2% per samples")) +ggtitle("Family") +
  theme( axis.title.x = element_text(color="black", size=11, face="bold")) + 
  theme(strip.text = element_text(size = 12, face = "bold")) +
  theme(legend.text = element_text(size=12)) +
  theme(legend.title = element_text(size = 12, face= "bold")) +
...
genus <- read.csv("GenusBeef.csv", na.strings = c("","NA"), header=TRUE)
genus$Status = as.factor(genus$Status)
levels(genus$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
...
ggplot(genus,aes(x=Abundance.average,y=factor(Genus, levels = rev(levels(factor(Genus)))),fill=Status)) + 
  ...
  theme_bw()+ #scale_x_continuous(breaks = seq(0, 0.30, by = 0.30), limits = c(0, 0.30)) +
  theme(strip.text.y = element_text(angle = 0, face="italic", size=10)) +theme(axis.title.y = element_blank()) +
  xlab(paste0("Relative Abundance > 2% per samples")) +ggtitle("Genus") +
  theme( axis.title.x = element_text(color="black", size=11, face="bold")) + 
  theme(strip.text = element_text(size = 12, face = "bold")) +
  theme(legend.text = element_text(size=12)) +
  theme(legend.title = element_text(size = 12, face= "bold")) +
...
COT = subset(metadata, State == "CO")
CO_OTU <- NewASVTable[rownames(NewASVTable) %in% rownames(COT),]
IDT = subset(metadata, State == "ID")
ID_OTU <- NewASVTable[rownames(NewASVTable) %in% rownames(IDT),]
INT = subset(metadata, State == "IN")
IN_OTU <- NewASVTable[rownames(NewASVTable) %in% rownames(INT),]
TXT = subset(metadata, State == "TX")
TX_OTU <- NewASVTable[rownames(NewASVTable) %in% rownames(TXT),]

COH = subset(COT, Status== "Healthy")
CO_H <- NewASVTable[rownames(NewASVTable) %in% rownames(COH),] 
COH <- COH[rownames(COH) %in% rownames(CO_H),]
COB = subset(COT, Status== "BRD")
CO_B <- NewASVTable[rownames(NewASVTable) %in% rownames(COB),] 
COB <- COB[rownames(COB) %in% rownames(CO_B),]

INH = subset(INT, Status== "Healthy")
IN_H <- NewASVTable[rownames(NewASVTable) %in% rownames(INH),] 
INH <- INH[rownames(INH) %in% rownames(IN_H),]
INB = subset(INT, Status== "BRD")
IN_B <- NewASVTable[rownames(NewASVTable) %in% rownames(INB),] 
INB <- INB[rownames(INB) %in% rownames(IN_B),]

IDH = subset(IDT, Status== "Healthy")
ID_H <- NewASVTable[rownames(NewASVTable) %in% rownames(IDH),] 
IDH <- IDH[rownames(IDH) %in% rownames(ID_H),]
IDB = subset(IDT, Status== "BRD")
ID_B <- NewASVTable[rownames(NewASVTable) %in% rownames(IDB),] 
IDB <- IDB[rownames(IDB) %in% rownames(ID_B),]

TXH = subset(TXT, Status== "Healthy")
TX_H <- NewASVTable[rownames(NewASVTable) %in% rownames(TXH),] 
TXH <- TXH[rownames(TXH) %in% rownames(TX_H),]
TXB = subset(TXT, Status== "BRD")
TX_B <- NewASVTable[rownames(NewASVTable) %in% rownames(TXB),] 
TXB <- TXB[rownames(TXB) %in% rownames(TX_B),]

#### ---- identify the taxonomy composition within each state
#modify every time you test a different state
OTU.physeqS = otu_table(as.matrix(TX_B), taxa_are_rows=FALSE)
tax.physeqS = tax_table(as.matrix(NewTax))
#meta.physeq = sample_data(meta)
meta.physeqS = sample_data(TXB) #modify every time you test a different state
...
COG_H =physeq_meta_filtered
COG_B =physeq_meta_filtered
COG= rbind(COG_H, COG_B)
COG$Status = as.factor(COG$Status)
levels(COG$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
COG$State = "CO"

ING_H =physeq_meta_filtered
ING_B =physeq_meta_filtered
ING= rbind(ING_H, ING_B)
ING$Status = as.factor(ING$Status)
levels(ING$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
ING$State = "IN"

IDG_H =physeq_meta_filtered
IDG_B =physeq_meta_filtered
IDG= rbind(IDG_H, IDG_B)
IDG$Status = as.factor(IDG$Status)
levels(IDG$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
IDG$State = "ID"

TXG_H =physeq_meta_filtered
TXG_B =physeq_meta_filtered
TXG= rbind(TXG_H, TXG_B)
TXG$Status = as.factor(TXG$Status)
levels(TXG$Status) <- list("Healthy"="Healthy", "BRD"="BRD")
TXG$State = "TX"

## now plot the difference 
my_colors <- c("dodgerblue3","goldenrod3")

a = ggplot(COG,aes(x=Abundance.average,y=factor(Genus, levels = rev(levels(factor(Genus)))),fill=Status)) + 
  geom_bar(stat = "identity", width= 0.7, position="dodge") + scale_fill_manual(values = my_colors) +
  theme_bw()+ scale_x_continuous(breaks = seq(0, 0.30, by = 0.30), limits = c(0, 0.30)) +
  theme(strip.text.y = element_text(angle = 0, face="italic", size=10)) +theme(axis.title.y = element_blank()) +
  xlab(paste0("Relative Abundance > 2% per samples")) +ggtitle("CO Samples") +
  theme( axis.title.x = element_text(color="black", size=11, face="bold")) + 
  theme(strip.text = element_text(size = 12, face = "bold")) +
  theme(legend.text = element_text(size=12)) +  guides(fill="none") +
  theme(legend.title = element_text(size = 12, face= "bold")) +
  theme(legend.key.size = unit(12, "point")) +
  theme(axis.text.x = element_text(color = "black", size = 10), axis.text.y = element_text(color = "black", size = 12, face= "italic")) 
a

b = ggplot(IDG,aes(x=Abundance.average,y=factor(Genus, levels = rev(levels(factor(Genus)))),fill=Status)) + 
  geom_bar(stat = "identity", width= 0.7, position="dodge") + scale_fill_manual(values = my_colors) +
  theme_bw()+ scale_x_continuous(breaks = seq(0, 0.30, by = 0.30), limits = c(0, 0.30)) +
  theme(strip.text.y = element_text(angle = 0, face="italic", size=10),) +theme(axis.title.y = element_blank()) +
  xlab(paste0("Relative Abundance > 2% per samples")) +ggtitle("ID Samples") +
  theme( axis.title.x = element_text(color="black", size=11, face="bold")) + 
  theme(strip.text = element_text(size = 12, face = "bold")) +
  theme(legend.text = element_text(size=12)) +  guides(fill="none") +
  theme(legend.title = element_text(size = 12, face= "bold")) +
...
c = ggplot(ING,aes(x=Abundance.average,y=factor(Genus, levels = rev(levels(factor(Genus)))),fill=Status)) + 
  geom_bar(stat = "identity", width= 0.7, position="dodge") + scale_fill_manual(values = my_colors) +
  theme_bw()+ scale_x_continuous(breaks = seq(0, 0.30, by = 0.30), limits = c(0, 0.30)) +
  theme(strip.text.y = element_text(angle = 0, face="italic", size=10)) +theme(axis.title.y = element_blank()) +
  xlab(paste0("Relative Abundance > 2% per samples")) +ggtitle("IN Samples") +
  theme( axis.title.x = element_text(color="black", size=11, face="bold")) + 
  theme(strip.text = element_text(size = 12, face = "bold")) +
  theme(legend.text = element_text(size=12)) +  guides(fill="none") +
  theme(legend.title = element_text(size = 12, face= "bold")) +
...
d= ggplot(TXG,aes(x=Abundance.average,y=factor(Genus, levels = rev(levels(factor(Genus)))),fill=Status)) + 
  geom_bar(stat = "identity", width= 0.7, position="dodge") + scale_fill_manual(values = my_colors) +
  theme_bw()+ scale_x_continuous(breaks = seq(0, 0.30, by = 0.30), limits = c(0, 0.30)) +
  theme(strip.text.y = element_text(angle = 0, face="italic", size=10)) +theme(axis.title.y = element_blank()) +
  xlab(paste0("Relative Abundance > 2% per samples")) +ggtitle("TX Samples") +
  theme( axis.title.x = element_text(color="black", size=11, face="bold")) + 
  theme(strip.text = element_text(size = 12, face = "bold")) +
  theme(legend.text = element_text(size=12)) +  guides(fill="none") +
  theme(legend.title = element_text(size = 12, face= "bold")) +
...
ggarrange(d,ncol =1, labels = c("D"),
          font.label = list(size = 18))

GenusTotal = rbind(COG, ING, IDG, TXG)
write.csv(GenusTotal, "GenusTotalStateBeef.csv")
...
patho$Species = as.factor(patho$Species)
levels(patho$Species)
patho <- patho[patho$Species %in% c(" Bibersteinia", " Mesomycoplasma_dispar",
                                    " Mycoplasma", " Mycoplasmopsis_bovirhinis", " Pasteurella_multocida",
                                    " Histophilus", " Mannheimia_haemolytica", " Trueperella"), ]
levels(patho$Species)
...
#SpeciesS = Species
SpeciesS = rbind(SpeciesS, Species)
write.csv(SpeciesS, "BeefStateBRDSpecies.csv")
