...
x = as.data.frame(y_Crohn)

coda_glmnet_Crohn<-coda_glmnet(x=x_Crohn,y=y_Crohn)

coda_glmnet_Crohn2<-coda_glmnet_null(x=x_Crohn,y=y_Crohn, niter=2,covar=NULL,lambda="lambda.1se",
                                     alpha=0.9,sig=0.05)
summary(coda_glmnet_Crohn2$"accuracy")


cv.glmnet(coda_glmnet_Crohn)
...
prunetable$GenusASV = paste(prunetable$Genus, prunetable$OTU, sep = '-')
NewTax <- prunetable[,c(1:9, 412)]
row.names(NewTax) <- NewTax[,10]
NewTax = NewTax[,-c(10)]

NewASVTable <- prunetable[-c(1:9)]
row.names(NewASVTable) <- NewASVTable[,403]
NewASVTable = NewASVTable[,-c(403)]
NewASVTable = t(NewASVTable)


#check if the rownames are the same
a <- rownames(NewASVTable)
metadata <- metadata[rownames(metadata) %in% rownames(NewASVTable),]
b <- rownames(metadata)
identical(a,b)

sample_id <- metadata[6] #subset just the sample info
sample_id$Status <- as.factor(sample_id$Status)

###now subset by the BRD and healthy data
#Healthy 
...
otu_abund <- colSums(H_OTU) ##the abundance of each ASV across all samples
H_OTU = t(H_OTU)
...
NewASV <- t(NewASV[rownames(NewASV) %in% rownames(patho),])

NewNames = sample_id[rownames(sample_id) %in% rownames(NewASV ),]
summary(NewNames)

####
coda_glmnetDairy<-coda_glmnet(x=NewASV,y=NewNames)

coda_glmnet_Dairy2<-coda_glmnet_null(x=NewASV,y=NewNames, niter=2,covar=NULL,lambda="lambda.1se",
                                     alpha=0.9,sig=0.05)
summary(coda_glmnet_Dairy2$"accuracy")

cv.glmnet(coda_glmnetDairy)
coda_glmnetDairy$taxa.num
length(coda_glmnetDairy$taxa.num)  # number of selected taxa

coda_glmnetDairy$taxa.name
coda_glmnetDairy$`log-contrast coefficients`
sum(coda_glmnetDairy$`log-contrast coefficients`)
coef<-coda_glmnetDairy$`log-contrast coefficients`
positives<-which(coef>=0)
op<-order(coef[positives], decreasing = TRUE)
coda_glmnetDairy$taxa.name[positives[op]]

negatives<-which(coef<0)
on<-order(abs(coef[coef<0]), decreasing = TRUE)
coda_glmnet_Crohn$taxa.name[negatives[on]]

coda_glmnetDairy$`signature plot`

#prediction output
coda_glmnetDairy$`predictions plot`
