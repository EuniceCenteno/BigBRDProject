...
ggplot(data=meta2, aes(x=Bacteria, y=copy_log)) +
  ...
  geom_boxplot(aes(x=Bacteria, y=copy_log,fill= Status), position=position_dodge(width=0.8), alpha=0.4) +
...
## total abundance of status and bacteria
summary6 <-meta2%>%
...
level_to_remove <- "P.multocida"
qPCR <- qPCR[qPCR$Bacteria != level_to_remove, ]
...
ggplot(data=qPCR, aes(x=Status, y=copy_log)) +
  ...
  geom_boxplot(aes(x=Status, y=copy_log,fill= Status), alpha=0.5) +
...
meta2%>%
  ggplot( aes(x=Bacteria, y=copy_log, fill=Status)) +
