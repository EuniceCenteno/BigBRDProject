...
ggplot(data=metadata3, aes(x=Status, y=value)) +
  ...
  geom_boxplot(aes(x=Status, y=value,fill= Status), alpha=0.5) +
...
a = ggplot(metadata, aes(x=State, y=observed_otus)) +
  geom_boxplot(aes(x=State, y=observed_otus,fill=State),  alpha= 0.9) +
  #facet_grid(.~Bacteria) + 
...
a
b = ggplot(metadata, aes(x=State, y=chao1)) +
  geom_boxplot(aes(x=State, y=chao1,fill=State),  alpha= 0.9) +
  #facet_grid(.~Bacteria) + 
  scale_fill_manual(values = my_color) + theme_bw() +
  stat_summary(fun.y=mean, geom="point", shape=17, size=2, color="red", fill="red")+
  geom_jitter(aes(x=State, y=chao1,color=State), width = 0.25, alpha=0.6, shape=19) +
...
c = ggplot(metadata, aes(x=State, y=pielou_e)) +
  geom_boxplot(aes(x=State, y=pielou_e,fill=State),  alpha= 0.9) +
  #facet_grid(.~Bacteria) + 
  scale_fill_manual(values = my_color) + theme_bw() +
  geom_jitter(aes(x=State, y=pielou_e,color=State), width = 0.25, alpha=0.6, shape=19) +
  ylab("Pielou_e") +xlab ("State") + 
  stat_summary(fun.y=mean, geom="point", shape=17, size=2, color="red", fill="red")+
  scale_color_manual(values = my_color) + guides(color="none") +
  #stat_compare_means(label.y =1150,label.x = 1.1, size=3.5)+ theme(legend.position = "none") +
  stat_compare_means(comparisons = my_comparisons, label = "p.signif")+
...
d = ggplot(metadata, aes(x=State, y=shannon)) +
  geom_boxplot(aes(x=State, y=shannon,fill=State),  alpha= 0.9) +
  #facet_grid(.~Bacteria) + 
  scale_fill_manual(values = my_color) + theme_bw() +
  geom_jitter(aes(x=State, y=shannon,color=State), width = 0.25, alpha=0.6, shape=19) +
...
e = ggplot(metadata, aes(x=State, y=faith_pd)) +
  geom_boxplot(aes(x=State, y=faith_pd,fill=State),  alpha= 0.9) +
  #facet_grid(.~Bacteria) + 
  scale_fill_manual(values = my_color) + theme_bw() +
  geom_jitter(aes(x=State, y=faith_pd,color=State), width = 0.25, alpha=0.6, shape=19) +
  ylab("Faith PD") +xlab ("State") + 
  scale_color_manual(values = my_color) + guides(color="none") +
  stat_summary(fun.y=mean, geom="point", shape=17, size=2, color="red", fill="red")+
  #stat_compare_means(label.y =1150,label.x = 1.1, size=3.5)+ theme(legend.position = "none") +
  stat_compare_means(comparisons = my_comparisons, label = "p.signif")+
...
ggplot(data=metadata2, aes(x=State, y=value)) +
  ...
  geom_boxplot(aes(x=State, y=value,fill= Status), alpha=0.5) +
  scale_fill_manual(values = my_colors) +
  facet_free(alpha~.) + scale_color_manual(values = my_colors) + theme_bw() +
  theme_bw() + ylab("Alpha Diversity Metrics") +
  theme(legend.title = element_text(size=12, face="bold")) +
...
shannon <-read.table("~/Desktop/eunice/PhD/AgSEED/BigProject/BRDQiime/QiimeBeef/CorrectBeefQiime/exported/shannon.tsv", header=TRUE, row.names=1, sep="\t")
...
chao1 <- read.table("~/Desktop/eunice/PhD/AgSEED/BigProject/BRDQiime/QiimeBeef/CorrectBeefQiime/exported/chao1.tsv", header=TRUE, row.names=1, sep="\t")
...
a = ggplot(metadata2, aes(x=State, y=observed_otus)) +
  ...
  theme(axis.text.x = element_blank(), axis.text.y =
          element_text(color = "black", size = 9))
a
b = ggplot(metadata2, aes(x=State, y=chao1)) +
  geom_boxplot(aes(x=State, y=chao1,fill=State),  alpha= 0.9) +
  #facet_grid(.~Bacteria) + 
  scale_fill_manual(values = my_color2) + theme_bw() +
  geom_jitter(aes(x=State, y=chao1,color=State), width = 0.25, alpha=0.6, shape=19) +
...
d = ggplot(metadata2, aes(x=State, y=shannon)) +
  geom_boxplot(aes(x=State, y=shannon,fill=State),  alpha= 0.9) +
  #facet_grid(.~Bacteria) + 
  scale_fill_manual(values = my_color2) + theme_bw() +
  geom_jitter(aes(x=State, y=shannon,color=State), width = 0.25, alpha=0.6, shape=19) +
