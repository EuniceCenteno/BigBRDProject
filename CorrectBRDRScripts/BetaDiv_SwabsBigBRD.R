...
######################Beef samples
setwd("~/Desktop/eunice/PhD/AgSEED/BigProject/BRDQiime/QiimeBeef/CorrectBeefQiime/") #sets new working directory for Windows systems (remember to replace … with your filepath)
...
ASVs <- read_qza("rarefied_table.qza") #17622 ASVs, 469 samples
ASV_s <- as.data.frame(ASVs$data)
ASV_table <- as.data.frame(ASVs$data) #17622 ASVs
ASV_table$ASVnos <- paste0("ASV", 1:nrow(ASV_table))
ASV_table$ASVstring <- rownames(ASV_table)
rownames(ASV_table) <- ASV_table$ASVnos ##We change the ASV name created in Qiime to ASVn
ASVkey <- ASV_table[, (ncol(ASV_table)-1):ncol(ASV_table)] #the key withe the names
...
ASV_table <- as.matrix(ASV_table)
x = as.data.frame(t(ASV_table[c(1,2),]))
sum(x$Beef1) #10,190 reads per sample
...
#We then merge these into an object of class phyloseq.
physeq_deseq = phyloseq(OTU.physeq, tax.physeq, meta.physeq)
physeq_deseq # [ 17622 taxa and 469 samples ] dairy
...
physeq_deseq %>%
  subset_taxa(Family != " Mitochondria" & 
                ...
                Genus != " Chloroplast" &
                Species != " Chloroplast") -> physeq_deseq
physeq_deseq #[ 17534 taxa and 469 samples ] Beef
...
wt.unifrac <- plot_ordination(physeq1, 
                              ordu.wt.uni, color="State") 
wt.unifrac <- wt.unifrac + ggtitle("WU ~ State") + geom_point(size = 0.5)
wt.unifrac <- wt.unifrac + theme_classic() 
print(wt.unifrac + stat_ellipse())
a = print(wt.unifrac + stat_ellipse() +
...
my_colors <- c("dodgerblue3","goldenrod3")
wt.unifrac2 <- plot_ordination(physeq1, 
                              ordu.wt.uni, color="Status") 
wt.unifrac2 <- wt.unifrac2 +  geom_point(size = 1)
wt.unifrac2 <- wt.unifrac2 + theme_bw() 
print(wt.unifrac2 + stat_ellipse())
b = print(wt.unifrac2 + stat_ellipse() + ggtitle("WU ~ Status") +
            ...
            theme(axis.text.x = element_text(color = "black", size = 10), axis.text.y = element_text(color = "black", size = 10)))
b
c = print(wt.unifrac2 + stat_ellipse() + ggtitle("WU ~ Status by State") +
            scale_color_manual(values = my_colors) + facet_wrap(State~.) +
            theme(legend.text = element_text(size=11)) +guides(color="none") +
            theme(legend.title = element_text(size = 11, face= "bold")) +
...
Bray <- plot_ordination(physeq1, 
                        ordu.bc, color="State") 
Bray <- Bray  + geom_point(size = 2)
Bray <- Bray + theme_classic() 
x = print(Bray + stat_ellipse() +ggtitle("BC ~ State") +
            scale_color_manual(values = my_color2) +
            theme(legend.text = element_text(size=11)) +
            theme(legend.title = element_text(size = 11, face= "bold")) +
            theme(axis.title.x = element_text(color="black", size=10, face="bold"), axis.title.y = element_text(color="black", size=10, face="bold")) + 
...
x
my_colors <- c("dodgerblue3","goldenrod3")
Bray2 <- plot_ordination(physeq1, 
                        ordu.bc, color="Status") 
Bray2 <- Bray2 +  geom_point(size = 1)
Bray2 <- Bray2 + theme_bw() 
print(Bray2 + stat_ellipse())
y = print(Bray2 + stat_ellipse() + ggtitle("BC ~ Status") +
            scale_color_manual(values = my_colors) +
            theme(legend.text = element_text(size=11)) +
            theme(legend.title = element_text(size = 11, face= "bold")) +
            theme(axis.title.x = element_text(color="black", size=10, face="bold"), axis.title.y = element_text(color="black", size=10, face="bold")) + 
            theme(axis.text.x = element_text(color = "black", size = 10), axis.text.y = element_text(color = "black", size = 10)))
y
z = print(Bray2 + stat_ellipse() + ggtitle("BC ~ Status by State") +
            scale_color_manual(values = my_colors) + facet_wrap(State~.) +
            theme(legend.text = element_text(size=11)) +guides(color="none") +
            theme(legend.title = element_text(size = 11, face= "bold")) +
            theme(strip.text = element_text(size = 11, face = "bold")) +
